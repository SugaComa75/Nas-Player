...
<body class="admin-page">
    ...
    <main class="admin-grid">
        ...
        <section class="card wide" id="admin-libraries" tabindex="-1">
            ...
            <form method="post" id="reorder-libraries-form"><input type="hidden" name="csrf" value="<?= tt_h(tt_csrf_token()) ?>"><input type="hidden" name="action" value="reorder_libraries"><div id="library-order-fields"></div><button type="submit" class="button primary">Save order</button></form>
        ...
        <section class="card" id="admin-users" tabindex="-1">
            ...
            <div class="table"><?php foreach ($users as $user): ?>
                    <div class="table-row">
                        <?php $userAvatarUrl = tt_avatar_url($user['avatar_url'] ?? ''); $userInitials = tt_initials((string) $user['display_name']); ?>
                        <div class="user-row-identity"><span class="avatar avatar-small" aria-hidden="true"><?php if ($userAvatarUrl): ?><img src="<?= tt_h($userAvatarUrl) ?>" alt="" onerror="this.hidden=true;this.nextElementSibling.hidden=false"><span class="avatar-fallback" hidden><?= tt_h($userInitials) ?></span><?php else: ?><span class="avatar-fallback"><?= tt_h($userInitials) ?></span><?php endif; ?></span><span><strong><?= tt_h($user['display_name']) ?></strong><small><?= tt_h($user['email']) ?> ·
